...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT report_count, last_report_time FROM place WHERE coordinates = ?");
    ...
    if ($place) {
        ...
        $lastReportTime = strtotime($place['last_report_time']);
        if (time() - $lastReportTime > 5 * 60) {
            // 신고 횟수 업데이트 및 마지막 신고 시간을 현재로 설정
            $updateStmt = $conn->prepare("UPDATE place SET report_count = report_count + 1, last_report_time = NOW() WHERE coordinates = ?");
            $updateStmt->bind_param("s", $coordinates);
            $updateStmt->execute();

            // report_comment 테이블에 신고 사유 입력
            if ($updateStmt->affected_rows > 0) {
                $reportStmt = $conn->prepare("INSERT INTO report_place (coordinates, report_reason, reporter_ip) VALUES (?, ?, ?)");
                $reportStmt->bind_param("sss", $coordinates, $reportReason, $user_ip);
                $reportStmt->execute();
            ...
            echo "이미 다른 이용자가 신고한 장소입니다.\n신고는 일정시간에 한번만 가능합니다.";
        }
    } else {
        echo "해당 장소를 찾을 수 없습니다.";
    }
